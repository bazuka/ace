...
(function() {
    ...
    this.$computeTabString = function() {
        ...
        if (this.displayIndentGuides) {
            this.$indentGuideRe = RegExp("^(\t+|(?: {" + tabSize + "})+)(\\s)");
            this.$tabStrings[" "] = this.$tabStrings[tabSize];
            this.$tabStrings["\t"] = this.showInvisibles 
                ? this.TAB_CHAR + this.$tabStrings[" "].substr(1)
                : this.$tabStrings[" "];
        }
    };
	...
	this.renderIndentGuide = function(stringBuilder, value) {
        var self = this;
        return value.replace(this.$indentGuideRe, function(_, a, b) {
            var cols = a[0] == " " ? a.length / 4 : a.length;
            stringBuilder.push(
                Array(cols + 1).join(
                    "<span class='ace_indent-guide ace_invisible' >"+
                        self.$tabStrings[a[0]]+
                    "</span>"
                )
            );
    ...
    this.renderIndentGuide2 = function(stringBuilder, value) {
        var re = /\s\S| \t|\t |\s$/
        var cols = value.search(re);
        if (!cols)
            return value;
        if (value[0] == " ") {
            cols -= cols % this.tabSize;
            stringBuilder.push(
                Array(cols/this.tabSize + 1).join(
                    "<span class='ace_indent-guide ace_invisible' >"+
                        this.$tabStrings[" "]+
                    "</span>"
                )
